...
impl SubMap
{
    ...
    fn to_mesh(&self, offset: Vec3, scale: f32, tileset: &TileSet, tiles: &Vec<TileData>) -> Mesh
    {
        ...
        for x in 0..self.width
        {
            ...
            {
                if let Some(base_verts) = self.at(x, y).get_base_verticies(vertex_offset, scale, tileset, tiles)
                {
                    verticies.extend_from_slice(&base_verts);
                    triangles.extend_from_slice(&self.at(x, y).get_triangles(triangle_offset));
                    triangle_offset += 4;
                }
                if let Some(top_verts) = self.at(x, y).get_top_verticies(vertex_offset, scale, tileset, tiles)
                {
...
impl TileData
{
    pub fn new(top: Option<UVec2>, base: Option<UVec2>, name: String, debug_name: String) -> TileData
    {
...
impl Tile
{
    ...
    fn get_top_verticies(&self, offset: Vec3, scale: f32, tileset: &TileSet, tiles: &Vec<TileData>) -> Option<[Vertex; 4]>
    {
        if let Some(id) = match self.data { Some(data) => tiles[data].top, None => None}
        {
    ...
    fn get_base_verticies(&self, offset: Vec3, scale: f32, tileset: &TileSet, tiles: &Vec<TileData>) -> Option<[Vertex; 4]>
    {
        if let Some(id) = match self.data { Some(data) => tiles[data].base, None => None}
        {
            let gen = |xoffset, yoffset|
            {
                let tile_size = Vec2{x: tileset.texture.width() / tileset.width as f32, y: tileset.texture.height() / tileset.height as f32};
...
impl TileMap
{
    ...
    fn at(&self, x: usize, y: usize) -> &Tile
    {
        ...
        let tiles = &self.map[map_y][map_x];
